...
use crate::{
    ...
    DisconnectedHeader, IndexedBlock,
};
...
impl<H: HeaderStore> Chain<H> {
    ...
    pub(crate) async fn locators(&mut self) -> Vec<BlockHash> {
        ...
        if !self.checkpoints_complete() {
            ...
            if self.header_chain.iter().count() < REORG_LOOKBACK as usize {
    ...
    pub(crate) async fn sync_chain(&mut self, message: Vec<Header>) -> Result<(), HeaderSyncError> {
        ...
        for header in header_batch.inner() {
                ...
                AcceptHeaderChanges::BlocksDisconnected {
                    ...
                    let disconnected_event = Event::BlocksDisconnected(
                        disconnected
                            .into_iter()
                            .rev()
                            .map(|index| DisconnectedHeader::new(index.height, index.header))
                            .collect(),
                    );
